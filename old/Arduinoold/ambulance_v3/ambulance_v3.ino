...
const int IgnDelay=2000, AlPowDelay=100, ntriggersDelay=1, SrnDuration=5000, 
          ...
          durationAledLED=2000, phoneInterval=1000, maxindex=250, phoneDiscardInterval=5000;

volatile int v_pirAl, v_pirTamp, v_Ign, ntriggers, readSMSOK=-99;
volatile bool Igndelay, IgnON, IsoSwCLOSED, HbrkSwDOWN;
unsigned long IgnTime=0,AlLEDTime=0, elapsedAlLED=0,
sinceIgn, isCPMSTime=0, phoneTime=0;
...
bool AlArmed=false, AlSwed=false, isTriggered=false, AlSwedON=false, SolSwedON=false,
 isCIND=false,isCMGR=false,isCPMS=false, delayOnce=true, prepareStorageAlreadyCalled=false;
int nAls=0, whichBattIsCharged=0, nSMS=0;
...
CPMS[10]="+CPMS: /0",smsReceived[16]="0791447728008/0", cmd[4];
  ...
  bool ended=false, ending=false, starting=false, started=false;
    ...
    tPhone.setInterval(2000, checkSMS);
    tPhone.setInterval(5000, checkStatus);
...
//end of packet
       if(character=='O'&&!ended) {
         ending=true; 
     //Serial.println(" ending");
   }
       if(character=='K'&&ending) {
      ended=true; 
    ending=false; 
...
Serial3.flush();  started=false; ended=false; starting=false; ending=false; 
tPhone.enable(0);
 }
  
  //if(ending) {Serial.print("ending");
  //Serial.print(starting);
  //Serial.print(started);
  //Serial.print(ending);
  //Serial.print(ended);
//Serial.println(millis()-phoneTime);}
     ...
     //if(ending) {Serial.print("ending");Serial.print(phoneTime); }
     
  if(started&&ended) {
    //Serial.print("started&&ended");
 ...
 isCPMS=false;
  ...
  if(phnData[i+j]==CPMS[j]) jCPMS++;
    ...
    if(jCIND==7) {isCIND=true; jCIND=i; break;}
    else if(jCMGR==7) {isCMGR=true; jCMGR=i;  break;}
    else if(jCPMS==7) {isCPMS=true; break;}
    //Serial.print(i);
  ...
  Serial.print("nSMS");Serial.println(nSMS);
  isCPMS=false; 
    }
    
    
    if(isCMGR) {
  for( j=index-10; j<index-6; j++) {
  //Serial.print(phnData[j]);
  cmd[j-(index-10)]=phnData[j];}
   cmd[4] = '\0';
   
  readSMSOK=encodeCommand(cmd);
 ...
 if(nSMS>0)
{
  ...
  if(isCIND) 
  {
    ...
    i--;
     ...
     isCIND=false;
     prepareStorageAlreadyCalled=false;
   }
