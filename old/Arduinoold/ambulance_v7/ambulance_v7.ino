...
//if (digitalRead(49)==LOW) chargeSolV();
 //if (digitalRead(51)==LOW) chargeSolL();
...
else if(phnBattLev>4) {nochargePhn(); nexe=94;}
...
if(pirTampTriggered){pirTampTriggered=false; 
		...
		if((digitalRead(p_pirTamp)==LOW)) 
		{nTrigs=maxnTrigs+1; Serial.print(" pirTampTriggered nTrigs:"); Serial.println(nTrigs);}
	}
	else if(HbrkSwTriggered){HbrkSwTriggered=false; 

		if(digitalRead(p_HbrkSw)==LOW) 
		{nTrigs=maxnTrigs+1; Serial.print(" HbrkSwDOWN nTrigs:"); Serial.println(nTrigs);}
	}
	else	if(pirAlTriggered){pirAlTriggered=false;  
	
		if((digitalRead(p_pirAl)==LOW)) 
		{nTrigs++; Serial.print(" pirAlTriggered nTrigs:"); Serial.println(nTrigs);}
	}

	//EXECUTE Al
if(nAls<maxnAls&&nTrigs>maxnTrigs) playSrn();
...
if(IsoSwTriggered) {IsoSwTriggered=false; 
	...
	if(IsoSwCLOSED)  { Serial.print("IsoSwCLOSED"); if(AlArmed&&nAls<maxnAls&&nTrigs>maxnTrigs) playSrn();}
